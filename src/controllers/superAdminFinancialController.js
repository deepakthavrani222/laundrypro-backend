...
class SuperAdminFinancialController {
  ...
  async getFinancialOverview(req, res) {
    try {
      ...
      // Get order-based revenue stats (same as dashboard)
      const orderStats = await Order.aggregate([
        ...
        },
        {
          $group: {
            ...
            totalDeliveryCharges: { $sum: '$pricing.deliveryCharge' },
      ...
      // Calculate platform fees (estimate 10% of revenue)
      const platformFees = Math.round(stats.totalRevenue * 0.10)

      // Get settlement stats from Orders (based on payment status)
      ...
      // Build settlement stats from payment status
      const settlementStats = {
        total: { count: stats.totalOrders, amount: stats.totalRevenue },
      ...
      paymentStats.forEach(ps => {
        if (ps._id === 'paid') {
          settlementStats.completed = { count: ps.count, amount: ps.amount }
        } else if (ps._id === 'pending') {
          settlementStats.pending = { count: ps.count, amount: ps.amount }
        } else if (ps._id === 'failed' || ps._id === 'refunded') {
          settlementStats.failed = { count: ps.count, amount: ps.amount }
        }
      })

      // Get pending approvals (orders with pending payment)
      ...
      // Get revenue trend from orders - format dates nicely
      const revenueTrend = await Order.aggregate([
        ...
        },
        {
          $group: {
            ...
            transactions: { $sum: 1 },
            fees: { $sum: { $multiply: ['$pricing.total', 0.10] } }
          }
        },
        { $sort: { dateSort: 1 } },
        { $project: { _id: 1, revenue: 1, transactions: 1, fees: 1 } },
        { $limit: timeframe === '7d' ? 7 : timeframe === '30d' ? 30 : 90 }
      ])
        ...
        data: {
          overview: {
            ...
            pendingApprovals: {
              ...
              settlements: 0,
              totalAmount: pendingAmount[0]?.total || 0
            }
          },
          revenueTrend,
          timeframe
...
module.exports = new SuperAdminFinancialController()
