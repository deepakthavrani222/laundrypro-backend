...
const centerAdminAdminsController = {
  // Get all admins (admin and center_admin roles)
  getAllAdmins: async (req, res) => {
    try {
      const admins = await User.find({ 
        role: { $in: ['admin', 'center_admin'] }
      })
      .select('-password')
  ...
  createAdmin: async (req, res) => {
    try {
      const { name, email, phone, password, role, permissions, assignedBranch } = req.body
      ...
      // Validate role
      const validRoles = ['admin', 'center_admin']
      if (role && !validRoles.includes(role)) {
        return res.status(400).json({
          success: false,
          message: 'Invalid role. Must be admin or center_admin'
        })
      }
      ...
      // For center_admin, validate branch assignment
      ...
      const admin = new User({
        ...
        role: role || 'admin',
        permissions,
        assignedBranch: assignedBranch || undefined,
        isActive: true
      })
      ...
      res.status(201).json({
        success: true,
        message: `${role === 'center_admin' ? 'Center Admin' : 'Admin'} created successfully`,
  ...
  // Invite Admin (new invitation-based flow)
  inviteAdmin: async (req, res) => {
    try {
      const { email, role, permissions, assignedBranch } = req.body

      // Validate required fields
      if (!email || !role || !permissions) {
        return res.status(400).json({
          success: false,
          message: 'Email, role, and permissions are required'
        })
      }
      ...
      // Check if email already exists as admin or center_admin
      ...
      const invitation = new AdminInvitation({
        ...
        invitedByModel: req.admin.role === 'superadmin' ? 'SuperAdmin' : 'CenterAdmin'
      })
